  ...
  return (
    <>
      ...
      <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {(data ?? []).map((item: any) => (
          <a key={item._id} href={`/vs/${item.slug?.current || ''}`} className="group block rounded-xl border border-bw-gray-200 bg-bw-gray-700 p-6 card-hover">
            <h3 className="text-lg font-semibold text-bw-gray-800 group-hover:text-bw-yellow-500 transition-colors">{item.title}</h3>
            {item.metaDescription && <p className="mt-2 text-sm text-bw-gray-600 line-clamp-2">{item.metaDescription}</p>}
          </a>
        ))}
        </div>
        {(data ?? []).length === 0 && (
