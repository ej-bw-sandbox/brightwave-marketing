...
export const metadata: Metadata = {
  ...
  description: 'Technical deep dives from the Brightwave engineering team.',
}
...
export default async function Page() {
  ...
  try {
    const data = await client.fetch(
      ...
      { category: 'engineering-log', offset: 0, limit: 50 },
      ...
      <section className="c-section cc-template">
        <div className="c-container">
          <div className="bp40-underline">
            <h1 className="c-title-1">Engineering Log</h1>
          </div>
          <div className="w-dyn-list">
            {featured ? (
              <div role="list" className="w-dyn-items">
                <div role="listitem" className="card_item w-dyn-item">
                  <Link href={`/engineering-log/${featured.slug?.current || ''}`} className="grid cc-featured w-inline-block">
                    ...
                    <div id="w-node-_8d9fda5a-b3e5-2f57-492c-719e340eb766-4c4a8e00" className="aspect-16-9 u-overflow-hidden">
                      {featured.coverImage?.asset ? (
                        <img src={urlFor(featured.coverImage).width(1200).height(630).fit('crop').url()} loading="lazy" alt={featured.title || ''} className="img-cover" />
                      ) : (
                    ...
                    <div id="w-node-e443da8c-33db-3129-61f5-1e4331df01f6-4c4a8e00" className="featured_right">
                      <div className="featured_top">
                        <div className="eyebrow-flex">
                          ...
                          <div className="c-title-5">Engineering Log</div>
                        </div>
                ...
                <div role="list" className="grid cc-collection w-dyn-items">
                  {rest.map((post: any) => (
                    <div key={post._id} id="w-node-c5123b81-e463-2eab-695f-359de2f2994d-4c4a8e00" role="listitem" className="collection_card w-dyn-item">
                      <Link href={`/engineering-log/${post.slug?.current || ''}`} className="card w-inline-block">
                        <div className="aspect-4-3">
                          {post.coverImage?.asset ? (
                            <img alt={post.title || ''} loading="lazy" src={urlFor(post.coverImage).width(1200).height(630).fit('crop').url()} className="img-cover" />
                          ) : (
