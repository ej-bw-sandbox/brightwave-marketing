  ...
  return (
    <>
      ...
      <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {(data ?? []).map((item: any) => (
          <a key={item._id} href={`/events/${item.slug?.current || ''}`} className="group block rounded-xl border border-bw-gray-600 bg-bw-gray-700 p-6 card-hover">
            <time className="text-xs text-bw-gray-300">{item.eventDate ? new Date(item.eventDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : ''}</time>
            <h3 className="mt-2 text-lg font-semibold text-bw-gray-50 group-hover:text-bw-yellow-500 transition-colors">{item.title}</h3>
            {item.description && <p className="mt-2 text-sm text-bw-gray-200 line-clamp-2">{item.description}</p>}
          </a>
        ))}
        </div>
        {(data ?? []).length === 0 && (
